...
def select_features(train_raw,node_raw,train_idx,fdir,meta,disease,fn):
...
def detect_dsp(graph, node_raw,feature_id, labels_raw,labels,adj, train_id, test_id, rdir,ot2,classes_dict, tid2name, wwl,close_cv,sid,sname):
    setup_seed(10)
    dn={}
...
def feature_importance_check(selected,selected_arr,feature_id,train_idx,val_idx,test_idx,features,adj,labels,rdir,fn,classes_dict,tid2name,o3,wwl,ot,dcs,fnum,close_cv,o4,node_raw,oin):
    setup_seed(10)
    cround=1
    top100={}
    idx_features_labels = np.genfromtxt("{}".format(node_raw), dtype=np.dtype(str))
...
def run(node_norm,train_raw,node_raw,meta,disease,out,kneighbor,pre_features,rseed,cvfold,doadpt,insp,fnum,nnum,close_cv,anode,reverse,vnode,uf,bsize,rfi,oin):
    if not rseed==0:
        setup_seed(rseed)
    ...
    for train_idx,val_idx in datasets:
        ...
        if uf==0:
            if len(pre_features)==0:
                eg_fs_sf=select_features(train_raw,node_raw,train_idx,fdir,meta,disease,fn+1)
            else:
    ...
    if rfi==1:
        ...
        uid = uuid.uuid1().hex
        ot = open(uid + '.log', 'w+')
        sid,sname=feature_importance_check(selected, selected_arr, feature_id, bset[0], bset[1], test_idx, features, bset[3],bset[2], rdir, bset[4], classes_dict, tid2name, o3, wwl, ot, dcs, fnum, close_cv,o4,node_raw,oin)
        ot.close()
        os.system('rm ' + uid + '.log')
        #### Biomarker influence score
        uid = uuid.uuid1().hex
        ot2 = open(uid + '.log', 'w+')
        detect_dsp(btgraph, node_raw,feature_id, labels_raw,bset[2],bset[3], tem_train_id, test_idx, rdir,ot2,classes_dict, tid2name, wwl,close_cv,sid,sname)
        ot2.close()
        os.system('rm ' + uid + '.log')
    #exit()
